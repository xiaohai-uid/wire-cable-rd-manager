...
- [ ] Vite + TypeScript(strict) + React + Tailwind + Vitest 骨架可跑，`npm test` 与 `npm run dev` 都通
- [ ] 现有 52 条真实规格串逐条断言解析结果（种类、阈值、单位）全部通过
- [ ] `≥1.5mm2` 与 `≥1.5 mm2` 的阈值解析为 `1.5`，单位为 `mm2`（不是阈值 1.52）
- [ ] `≥10^8Ω`、`≥1×10^8Ω`、`2.5mm2±0.1` 均判为 `unparseable`
- [ ] 数值项判定规则：三次测量全部落在规格内才算合格；只填部分时按已填的判
- [ ] 测量值 `0` 参与判定，不被当作未填；未填只能是 `null`
- [ ] 三次全未填返回「未判定」，且不进入不良率分母
- [ ] 规格 `unparseable` 时判定拒绝给出合格/不合格结论
- [ ] 单值越界判断函数可用（供 UI 只标红越界的那一次测量）
- [ ] 热力图聚合函数以真实 62 条记录为夹具，总体不良率断言为 4.84%
- [ ] 三条已知不合格项断言通过：RV-0.5 导体电阻超 0.5Ω、SYV-75-5 回波损耗低于 20dB、BV-2.5 老化测试低于 80%
- [ ] 领域核心模块无任何 I/O、DOM、框架依赖（可用依赖检查或 import 审查确认）
